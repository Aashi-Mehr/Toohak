...
// Test function : adminQuizList Ver.1
describe('adminQuizList Version 1', () => {
  let userId: Token;
  let quiz: QuizList | ErrorObject;
  let quizId: number;

  test('INVALID User Id: Type of string or negative number', () => {
    expect(() => requestQuizList('123321', true)).toThrow(HTTPError[401]);
    expect(() => requestQuizList(-1, true)).toThrow(HTTPError[401]);
  });

  test('INVALID User Id: Not in the data base', () => {
    userId = requestRegister('first.last1@gmail.com', 'abcd1234', 'first',
      'last');
    expect(() => requestQuizList(11, true)).toThrow(HTTPError[401]);
  });
  ...
  test('VALID 1 Quiz', () => {
    ...
    quiz = requestQuizList(userId.token, true);
  ...
  test('VALID After Removal', () => {
    ...
    const result = requestQuizList(userId.token, true);
    expect(result).toMatchObject({ quizzes: [] });
  });
});

// Test function : adminQuizList Ver.2
describe('adminQuizList Version 2', () => {
  let userId: Token;
  let quiz: QuizList | ErrorObject;
  let quizId: number;
...
// Test function : adminQuizInfo Ver.1
describe('adminQuizInfo Version 1', () => {
  let userId: Token;
  let quiz: QuizInfo | ErrorObject;
  let quizId: number;

  test('INVALID User Id: Not a number or out of range number', () => {
    expect(() => requestQuizInfo('123321', 1, true)).toThrow(HTTPError[401]);
    expect(() => requestQuizInfo(-1, 1, true)).toThrow(HTTPError[401]);
  });
  ...
  test('INVALID User Id: Invalid quizId or quizId not matching', () => {
    ...
    expect(() => requestQuizInfo(2, 2, true)).toThrow(HTTPError[401]);
  });
  ...
  test('INVALID User Id: UserId does not exist', () => {
    ...
    expect(() => requestQuizInfo(4, 1, true)).toThrow(HTTPError[401]);
  });
  ...
  test('Test Valid User and Quiz Ids', () => {
    ...
    expect(requestQuizInfo(userId.token, quizId, true)).toMatchObject({
      quizId: quizId,
      name: 'QuizName',
  ...
  test('Test successful quiz read - correct timestamp format', () => {
    ...
    quiz = requestQuizInfo(userId.token, quizId, true);
    expect(quiz).toMatchObject({
      quizId: quizId,
      name: 'first last',
      timeCreated: expect.any(Number),
  ...
  test('Test quizId invalid error, cannot read', () => {
    ...
    expect(() => requestQuizInfo(
      userId.token, quizId + 1, true
    )).toThrow(HTTPError[403]);
  });
});

// Test function : adminQuizInfo Ver.2
describe('adminQuizInfo Version 2', () => {
  let userId: Token;
  let quiz: QuizInfo | ErrorObject;
  let quizId: number;
