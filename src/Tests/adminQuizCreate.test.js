...
test('Test Non-existing AuthUserId', () => {
  adminAuthRegister('validEmail@gmail.com', 'Val1dPassword', 'first', 'last');

  // Correct format authUserId but does not exist in data base
  const result = adminQuizCreate(0, 'QuizName', 'description');
  expect(result).toMatchObject({ error: expect.any(String) });
});
...
test('Test Quiz Name Is Already Used', () => {
  ...
  const result = adminQuizCreate(authId, 'quizName1', 'quizDescription1');
  expect(result).toMatchObject({ error: expect.any(String) });
});
