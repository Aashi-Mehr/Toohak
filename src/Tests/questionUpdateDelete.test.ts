...
describe('adminQuizQuestionUpdate', () => {
    ...
    let quizId2 = requestQuizCreate(token1, 'New Quiz 2', '').quizId;
    let questionId2 = requestQuestionCreate(token1, quizId2, finalQ).questionId;
  ...
  test('Question string is less than 5 characters in length', () => {
	...
	test('Valid token is provided, but the user is unauthorised', () => {
		...
		let token2 = requestRegister('a@gmail.com', 'Val1Pass', 'fir', 'las').token;
	...
	test('Moving, then updating', () => {
		...
		let question2 = requestQuestionCreate(token1, quizId1, initialQ);
